...
async function fullFixture(): Promise<{
  ...
  rcrt: RecruitTelegram;
}> {
  const rcrtFactory = await ethers.getContractFactory('RecruitTelegram');
  const rcrt: RecruitTelegram = await rcrtFactory.deploy();
  const { snakeSoldiers, passport, elementGem, factionGem, skillGem, catalog } =
    await deployContracts(rcrt.address, true);
  ...
  return { snakeSoldiers, elementGem, factionGem, skillGem, passport, catalog, renderUtils, rcrt };
}
...
describe('SnakeSoldiers', async () => {
  ...
  describe('With open sale', async () => {
    beforeEach(async () => {
      ...
      // Nedeed so it can mint:
      await rcrt.recruit(issuer.address);
    });
    ...
    describe('Access control', async () => {
      ...
      it('cannot open for all if not owner', async function () {
        await expect(snakeSoldiers.connect(notOwner).openForAll()).to.be.revertedWithCustomError(
          snakeSoldiers,
          'RMRKNotOwner',
        );
      });
    ...
    describe('Only RCRT holders', async () => {
      it('cannot mint if not RCRT holder', async function () {
        await expect(
          snakeSoldiers
            .connect(buyer)
            .mint(buyer.address, 4, SOLDIER_RANK, { value: SOLDIER_PRICE.mul(4) }),
        ).to.be.revertedWithCustomError(snakeSoldiers, 'MustBeRecruited');
      });

      it('can mint if RCRT holder', async function () {
        await rcrt.recruit(buyer.address);
        await snakeSoldiers
          .connect(buyer)
          .mint(buyer.address, 4, SOLDIER_RANK, { value: SOLDIER_PRICE.mul(4) });
    ...
    describe('With open sale for all', async () => {
      beforeEach(async () => {
        await snakeSoldiers.connect(issuer).openForAll();
      });
      ...
      it('cannot mint with the wrong price', async function () {
        await expect(
          snakeSoldiers
            .connect(buyer)
            .mint(buyer.address, 1, GENERAL_RANK, { value: SOLDIER_PRICE }),
      ...
      describe('With minted snakes', async () => {
        ...
        it('can get tokenURI (backwards compatibility)', async function () {
          expect(await snakeSoldiers.tokenURI(1)).to.eql(`${BASE_URI_EGGS}/general/generic`);
        });
        ...
        it('can reveal egg', async function () {
          ...
          expect(await renderUtils.getExtendedActiveAssets(snakeSoldiers.address, 1)).to.eql([
            [ASSET_ID_GENERAL_EGG, bn(0), `${BASE_URI_EGGS}/general/generic`],
          ]);
          ...
          expect(await renderUtils.getExtendedActiveAssets(snakeSoldiers.address, 21)).to.eql([
            [ASSET_ID_COMMANDER_EGG, bn(0), `${BASE_URI_EGGS}/commander/generic`],
          ]);
          ...
          expect(await renderUtils.getExtendedActiveAssets(snakeSoldiers.address, 201)).to.eql([
            [ASSET_ID_SOLDIER_EGG, bn(0), `${BASE_URI_EGGS}/soldier/generic`],
          ]);
        ...
        it('cannot withdraw raised funds if not owner', async function () {
          const expectedBalance = SOLDIER_PRICE.mul(4)
        ...
        describe('Gems', function () {
          ...
          it('can claim gems', async function () {
            ...
            expect(await elementGem.tokenURI(tokenId)).to.eql(`${BASE_URI_GEMS}/elements/earth`);
            expect(await factionGem.tokenURI(tokenId)).to.eql(`${BASE_URI_GEMS}/factions/mountain`);
            expect(await skillGem.tokenURI(tokenId)).to.eql(`${BASE_URI_GEMS}/skills/tank`);
          });
          ...
          describe('With claimed gems', function () {
            ...
            it('assigns gems to commanders as expected (skill matches element)', async function () {
              ...
              expect(await skillGem.getAssetMetadata(21, 1)).to.eql(`${BASE_URI_GEMS}/skills/tank`);
              ...
              expect(await skillGem.getAssetMetadata(25, 1)).to.eql(`${BASE_URI_GEMS}/skills/tank`);
            });
